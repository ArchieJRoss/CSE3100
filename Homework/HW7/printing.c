...
void * printer_main(void * arg)
{
    ...
    //init a variable for the printer (thread) object.
    printer_t *my_print = arg;
    
    //init variable for job queue.
    ...
    while(! done){
        ...
        if(r <= 0){
            //unlock the mutex only if all the jobs are done.
            pthread_mutex_unlock(&jq->mutex);
            done = 1;
            continue;
        }

        //otherwise fetch the job and unlock the mutex.
        int job = q_fetch_job(jq, my_print->id);
        pthread_mutex_unlock(&jq->mutex);
       
        //NOTE: I/O jobs like printing do not need to be in critical section.
        print_job(job);
        my_print->njobs++;
...
int main(int argc, char *argv[])
{
    ...
    // init a mutex variable object.
    if(pthread_mutex_init(&job_queue.mutex, NULL) != 0){
        perror("Failed to init mutex.\n");
    }

    // variable to initialize threads and arguments.
    int rv; 
